							...
							foreach (JsonValue conceptProperty in conceptGroup["conceptProperties"]) {
								conceptProperties.Add(
									new Model.ConceptProperty(){
										Name = conceptProperty.ContainsKey("name") ? (string)conceptProperty["name"] : String.Empty,
										Synonym = conceptProperty.ContainsKey("synonym") ? (string)conceptProperty["synonym"] : String.Empty
									});
							}
						}
					}
